...
const BUILD_TIME="2026-03-08T00:12:48Z";
    ...
    const mkGantt=(()=>{
      ...
      const bars=allT.map((t,i)=>{
        ...
        return `<g>
          ...
          ${isLate?`<text x="${Math.min(x2+4,W-20)}" y="${cy+4}" font-size="10" fill="#FF4444" font-weight="700">⚠ ${t.deadline}</text>`
            :t.deadline?`<text x="${Math.min(x2+4,W-50)}" y="${cy+4}" font-size="9" fill="#888">${t.deadline}</text>`:""}
        </g>`;
      }).join("");
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v73 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
