...
const BUILD_TIME="2026-03-08T00:40:20Z";
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v80 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
          ...
          {tab==="overview"&&(()=>{
            ...
            return(<div>
              <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:16,flexWrap:"wrap"}}>
                <h2 style={{margin:0,color:th.text,fontSize:17,fontWeight:800,flex:"0 0 auto"}}>Vista General</h2>
                <div style={{flex:1,minWidth:160,position:"relative"}}>
                  <span style={{position:"absolute",left:9,top:"50%",transform:"translateY(-50%)",fontSize:13,color:th.text5,pointerEvents:"none"}}>🔍</span>
                  <input value={searchQ} onChange={e=>setSearchQ(e.target.value)}
                    placeholder="Buscar tareas..." style={{...inp(th,{paddingLeft:28,fontSize:12,width:"100%"})}}/>
                </div>
                <select value={filterProj} onChange={e=>setFilterProj(e.target.value)}
                  style={{...inp(th,{fontSize:12,minWidth:140,cursor:"pointer"})}}>
                ...
                {(searchQ||filterProj)&&<button onClick={()=>{setSearchQ("");setFilterProj("");}}
                  style={{padding:"5px 10px",borderRadius:7,background:th.border,border:"none",color:th.text4,fontSize:11,cursor:"pointer"}}>✕ Limpiar</button>}
              </div>
              {(searchQ||filterProj)?(()=>{
                  ...
                  {matched.length>0&&<>
                    ...
                    {matched.map(t=>{
                      ...
                      return(<div key={t.id} style={{background:th.rowBg,borderRadius:10,padding:"10px 14px",marginBottom:6,border:`1px solid ${th.border2}`,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                        ...
                        {cat&&<span style={{fontSize:10,color:color.text,background:color.bg,padding:"2px 8px",borderRadius:99,flexShrink:0}}>{cat.icon} {cat.name}</span>}
