...
const BUILD_TIME="2026-03-08T00:22:56Z";
...
function mkTask(text){return{id:genId(),text,done:false,priority:"media",status:"backlog",blockReason:"",createdAt:today(),deadline:null,completedAt:null,jiraUrl:"",description:"",checklist:[],contacts:[],labels:[],owner:""};}
...
function exportProjectReport(cats,projects,dark,kpiData={},projFrom='',projTo=''){
  ...
  const buildKPISection=(proj,kpiData)=>{
    ...
    const tablesHtml=tables.map(table=>{
      ...
      const W=820,H=300,PAD={top:24,right:200,bottom:50,left:62};
      ...
      Object.entries(table.objectives||{}).forEach(([ci,val])=>{
        ...
        sp.push(`<line x1="${PAD.left}" x2="${PAD.left+innerW}" y1="${y}" y2="${y}" stroke="#FF4444" stroke-width="1.5" stroke-dasharray="6,3"/><line x1="${PAD.left+innerW}" x2="${PAD.left+innerW+8}" y1="${y}" y2="${y}" stroke="#FF4444" stroke-width="1.5"/><rect x="${PAD.left+innerW+10}" y="${y-9}" width="185" height="18" rx="4" fill="#FF444415" stroke="#FF4444" stroke-width="1"/><text x="${PAD.left+innerW+16}" y="${y+4}" text-anchor="start" font-size="10" font-weight="700" fill="#FF4444">${lbl}</text>`);
      });
      if(isLine){
        cols.forEach((col,ci)=>{
  ...
  const sections=projects.map(proj=>{
    ...
    const active=tasks.filter(t=>!t.done&&(t.status==="en-curso"||t.status==="escalado"||(t.deadline&&!t.status))).sort((a,b)=>(a.deadline||"z").localeCompare(b.deadline||"z"));
    ...
    const mkRow=(t,type)=>{
      ...
      const badge=type==="done"
        ...
        :`<div><span style="background:${sc.bg};color:${sc.c};padding:3px 10px;border-radius:99px;font-size:11px;font-weight:700;display:inline-block;">${statusLabel}</span>${tStatus==="bloqueado"&&t.blockReason?`<div style="margin-top:4px;font-size:10px;color:#CC2222;font-style:italic;">🔒 ${t.blockReason}</div>`:""}</div>`;
    ...
    // Build Gantt
    const mkGantt=(()=>{
      const allT=[...active,...blocked,...backlog,...completed].filter(t=>t.deadline);
      ...
      const xOff=d=>((new Date(d)-minD)/(1000*60*60*24))/totalDays*CHART_W;
      const STATUS_C={"en-curso":"#FF9F43",escalado:"#9966FF",bloqueado:"#FF4444",backlog:"#2E5FA3",done:"#6BCB77"};

      // Month header background
      const headerBg=`<rect x="${LABEL_W}" y="0" width="${CHART_W}" height="${HEADER_H}" fill="#EEF3FB"/>`;
      const labelBg=`<rect x="0" y="0" width="${LABEL_W}" height="${H}" fill="#F7FAFF"/>`;
      const chartBg=`<rect x="${LABEL_W}" y="${HEADER_H}" width="${CHART_W}" height="${H-HEADER_H-PAD_BOT}" fill="#FAFCFF"/>`;

      // Month grid lines & labels — all in HEADER_H zone
      const months=[];
      const cur=new Date(minD);
      cur.setDate(1);
      while(cur<=maxD){
        const x=LABEL_W+xOff(cur);
        months.push(`<line x1="${x}" x2="${x}" y1="${HEADER_H}" y2="${H-PAD_BOT}" stroke="#E0E8F4" stroke-width="1"/>`);
        months.push(`<text x="${x+4}" y="${HEADER_H-8}" font-size="10" font-weight="700" fill="#1A3A6E">${cur.toLocaleDateString("es-ES",{month:"short",year:"2-digit"})}</text>`);
        cur.setMonth(cur.getMonth()+1);
      }

      // Today line
      const todayX=LABEL_W+xOff(today_d);
      const todayLine=`<line x1="${todayX}" x2="${todayX}" y1="${HEADER_H}" y2="${H-PAD_BOT}" stroke="#FF4444" stroke-width="1.5" stroke-dasharray="4,3"/>
        <text x="${todayX+3}" y="${HEADER_H-8}" font-size="9" font-weight="700" fill="#FF4444">Hoy</text>`;

      // Divider between header and chart
      const divider=`<line x1="${LABEL_W}" x2="${W}" y1="${HEADER_H}" y2="${HEADER_H}" stroke="#D0D8EC" stroke-width="1.5"/>`;

      // Row stripes
      const stripes=allT.map((_,i)=>{
        const y=HEADER_H+i*ROW_H;
        return `<rect x="${LABEL_W}" y="${y}" width="${CHART_W}" height="${ROW_H}" fill="${i%2===0?"#FAFCFF":"#F3F7FC"}"/>`;
      }).join("");

      // Bars + labels
      const bars=allT.map((t,i)=>{
        const y=HEADER_H+i*ROW_H;
        const statusKey=t.done?"done":(t.status||"backlog");
        const color=STATUS_C[statusKey]||"#2E5FA3";
        const endD=new Date(t.deadline);
        const startD=t.createdAt?new Date(t.createdAt):new Date(minD);
        const x1=LABEL_W+Math.max(0,xOff(startD));
        const x2=LABEL_W+xOff(endD);
        const barW=Math.max(x2-x1,8);
        const isLate=!t.done&&t.deadline&&endD<today_d;

        // Word-wrap label into 2 lines of max 34 chars
        const words=t.text.split(" ");
        let ln1="",ln2="";
        for(const w of words){
          if((ln1+" "+w).trim().length<=34)ln1=(ln1+" "+w).trim();
          else if((ln2+" "+w).trim().length<=34)ln2=(ln2+" "+w).trim();
        }
        const cy=y+ROW_H/2;
        const labelEl=ln2
          ?`<text text-anchor="end" font-size="10" fill="#1A3A6E"><tspan x="${LABEL_W-8}" y="${cy-5}">${ln1}</tspan><tspan x="${LABEL_W-8}" dy="13">${ln2}</tspan></text>`
        ...
        const progressBar=pct!==null?`
          <rect x="${x1}" y="${y+6}" width="${barW}" height="${ROW_H-12}" rx="5" fill="${color}" opacity="0.22"/>
          <rect x="${x1}" y="${y+6}" width="${Math.max(barW*pct,pct>0?6:0)}" height="${ROW_H-12}" rx="5" fill="${color}" opacity="0.88"/>
          <text x="${x1+barW/2}" y="${cy+4}" text-anchor="middle" font-size="9" font-weight="700" fill="#fff" opacity="${barW>40?1:0}">${Math.round(pct*100)}%</text>
        `:` <rect x="${x1}" y="${y+6}" width="${barW}" height="${ROW_H-12}" rx="5" fill="${color}" opacity="${t.done?0.45:0.88}"/>`;

        // Checklist dots below bar (one dot per item)
        const dots=chkTotal>0&&chkTotal<=20?chkItems.map((c,di)=>{
          const done=c.state==="Completada"||c.done;
          const dx=x1+4+di*(Math.min(barW/chkTotal,12));
          return `<circle cx="${dx}" cy="${y+ROW_H-5}" r="3" fill="${done?"#6BCB77":"#ccc"}" opacity="0.9"/>`;
        }).join(""):"";
        ...
        return `<g>
          ...
          ${progressBar}
          ${dots}
          ${pct!==null?`<text x="${x1+barW+4}" y="${cy+4}" font-size="9" fill="${color}" font-weight="700">${Math.round(pct*100)}%</text>`:""}
          ${isLate?`<text x="${Math.min(x2+4,W-20)}" y="${y+ROW_H-5}" font-size="9" fill="#FF4444" font-weight="700">⚠ ${t.deadline}</text>`
            :t.deadline&&pct===null?`<text x="${Math.min(x2+4,W-50)}" y="${cy+4}" font-size="9" fill="#888">${t.deadline}</text>`:""}
        </g>`;
      }).join("");

      // Legend
      const legendItems=[["#2E5FA3","Backlog"],["#FF9F43","En curso"],["#9966FF","Escalado"],["#FF4444","Bloqueado"],["#6BCB77","Completado"]];
      const legend=legendItems.map(([c,l],i)=>`<g transform="translate(${LABEL_W+i*100},${H-10})"><rect x="0" y="-8" width="10" height="10" fill="${c}" rx="2" opacity=".88"/><text x="14" y="3" font-size="9" fill="#7A8AA0">${l}</text></g>`).join("");
      ...
      return `<div class="report-section" style="margin-bottom:24px;page-break-inside:avoid;break-inside:avoid;">
        ...
        <div style="overflow-x:auto;">
          <svg width="${W}" height="${H}" style="display:block;font-family:inherit;border-radius:8px;border:1px solid #E0E8F4;">
            ${labelBg}${chartBg}${headerBg}
            ${stripes}
            ${months.join("")}
            ${divider}
            ${todayLine}
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v75 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
