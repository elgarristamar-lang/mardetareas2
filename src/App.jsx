...
const BUILD_TIME="2026-03-07T19:11:14Z";
...
function mkTask(text){return{id:genId(),text,done:false,priority:"media",createdAt:today(),deadline:null,completedAt:null,jiraUrl:"",description:"",checklist:[],contacts:[],labels:[],owner:""};}
...
function exportProjectReport(cats,projects,dark,kpiData={}){
  const today_=today();
  // Collect all tasks with their labels
  const allTasks=cats.flatMap(c=>c.tasks
    .filter(t=>c.type!=="meeting"&&c.type!=="meeting121eq")
  ...
  const buildKPISection=(proj,kpiData)=>{
    ...
    const parseN=s=>parseFloat((s||"0").replace(/[.]/g,"").replace(",","."))||0;
    const tablesHtml=tables.map(table=>{
      const rows=(table.rows||[]).filter(r=>r&&r[0]);
      if(rows.length===0)return"";
      const cols=(table.graphCols||[]).map(ci=>table.columns[ci]);
      const W=620,H=230,PAD={top:20,right:20,bottom:44,left:56};
      const innerW=W-PAD.left-PAD.right;
      const innerH=H-PAD.top-PAD.bottom;
      const allVals=rows.flatMap(r=>(table.graphCols||[]).map(ci=>parseN(r[ci])));
      ...
      yTicks.forEach(v=>{
        ...
        svgParts.push('<line x1="'+PAD.left+'" x2="'+(PAD.left+innerW)+'" y1="'+y+'" y2="'+y+'" stroke="#E0E8F4" stroke-width="1"/>');
        svgParts.push('<text x="'+(PAD.left-5)+'" y="'+(y+4)+'" text-anchor="end" font-size="9" fill="#7A8AA0">'+fmtY(v)+'</text>');
      });
      Object.entries(table.objectives||{}).forEach(([ci,val])=>{
        if(!val)return;
        const y=PAD.top+yScale(parseFloat(val));
        svgParts.push('<line x1="'+PAD.left+'" x2="'+(PAD.left+innerW)+'" y1="'+y+'" y2="'+y+'" stroke="#FF4444" stroke-width="1.5" stroke-dasharray="6,3"/>');
        svgParts.push('<text x="'+(PAD.left+innerW-2)+'" y="'+(y-4)+'" text-anchor="end" font-size="8" fill="#FF4444">Obj '+table.columns[parseInt(ci)]+': '+parseFloat(val).toLocaleString("es-ES")+'</text>');
      });
      if(isLine){
        cols.forEach((col,ci)=>{
          const pts=rows.map((r,i)=>(PAD.left+i*xStep+xStep/2)+","+(PAD.top+yScale(parseN(r[(table.graphCols||[])[ci]])))).join(" ");
          svgParts.push('<polyline points="'+pts+'" fill="none" stroke="'+COLORS[ci%COLORS.length]+'" stroke-width="2.5"/>');
          rows.forEach((r,i)=>{
            const y=PAD.top+yScale(parseN(r[(table.graphCols||[])[ci]]));
            svgParts.push('<circle cx="'+(PAD.left+i*xStep+xStep/2)+'" cy="'+y+'" r="3.5" fill="'+COLORS[ci%COLORS.length]+'"/>');
          });
        });
      } else {
        rows.forEach((r,i)=>{
          ...
          cols.forEach((col,ci)=>{
            const v=parseN(r[(table.graphCols||[])[ci]]);
            const bh=Math.max((v/maxVal)*innerH,0);
            const bx=gX+ci*(barW+2);
            const by=PAD.top+innerH-bh;
            svgParts.push('<rect x="'+bx+'" y="'+by+'" width="'+barW+'" height="'+bh+'" fill="'+COLORS[ci%COLORS.length]+'" rx="2"/>');
          });
        });
      }
      rows.forEach((r,i)=>svgParts.push('<text x="'+(PAD.left+i*xStep+xStep/2)+'" y="'+(H-PAD.bottom+14)+'" text-anchor="middle" font-size="9" fill="#7A8AA0">'+r[0]+'</text>'));
      cols.forEach((col,ci)=>svgParts.push('<g transform="translate('+(PAD.left+ci*110)+','+(H-8)+')"><rect x="0" y="-7" width="10" height="10" fill="'+COLORS[ci%COLORS.length]+'" rx="2"/><text x="14" y="4" font-size="9" fill="#7A8AA0">'+col+'</text></g>'));
      const svg='<svg width="'+W+'" height="'+H+'" style="display:block;margin:0 auto;max-width:100%">'+svgParts.join("")+'</svg>';
      const thead='<tr style="background:#EEF3FB">'+table.columns.map(c=>'<th style="padding:6px 10px;text-align:left;font-size:11px;color:#1A3A6E;border-bottom:1px solid #E0E8F4">'+c+'</th>').join("")+'</tr>';
      const tbody=rows.map((r,i)=>'<tr style="background:'+(i%2===0?"#fff":"#FAFCFF")+'">'+r.map((c,ci)=>'<td style="padding:5px 10px;font-size:12px;border-bottom:1px solid #EEF2F8;font-weight:'+(ci===0?700:400)+';color:'+(ci===0?"#1A3A6E":"#1A1A2E")+'">'+c+'</td>').join("")+'</tr>').join("");
      return'<div style="margin-bottom:20px"><div style="font-size:12px;font-weight:700;color:#1A3A6E;margin-bottom:8px">'+table.name+'</div>'+svg+'<table style="width:100%;border-collapse:collapse;margin-top:10px;border:1px solid #E0E8F4;border-radius:8px;overflow:hidden"><thead>'+thead+'</thead><tbody>'+tbody+'</tbody></table></div>';
    }).join("");
  ...
  const sections=projects.map(proj=>{
    ...
    const active=tasks.filter(t=>!t.done&&t.deadline).sort((a,b)=>a.deadline.localeCompare(b.deadline));
    const backlog=tasks.filter(t=>!t.done&&!t.deadline);
    ...
    const mkRow=(t,type)=>{
      ...
      const badge=type==="done"
        ...
        :type==="active"
        ?`<span style="background:#FFF8E0;color:#AA8800;padding:2px 8px;border-radius:99px;font-size:10px;font-weight:700;">En curso${t.deadline?" · "+t.deadline:""}</span>`
        :`<span style="background:#F0F0F8;color:#666;padding:2px 8px;border-radius:99px;font-size:10px;font-weight:700;">Backlog</span>`;
      ...
      return`<tr>
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #EEF2F8;vertical-align:top;">${badge}</td>
      </tr>`;
    };
    ...
    return`
    <div class="project-section" style="margin-bottom:48px;page-break-inside:avoid;">
      <div style="background:linear-gradient(135deg,#1A3A6E,#2E5FA3);border-radius:12px;padding:20px 24px;margin-bottom:20px;color:#fff;display:flex;justify-content:space-between;align-items:center;">
        ...
        <div style="text-align:right;font-size:12px;opacity:.8;">
          <div>${completed.length} completadas · ${active.length} en curso · ${backlog.length} backlog</div>
          <div style="margin-top:4px;opacity:.6;">${today_}</div>
        </div>
      </div>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .main-header{background:linear-gradient(135deg,#0F2347,#1A3A6E);border-radius:16px;padding:28px 32px;color:#fff;margin-bottom:32px;display:flex;justify-content:space-between;align-items:center;}
...
<div class="page">
  <div class="main-header">
    <div>
      <div style="font-size:24px;font-weight:800;margin-bottom:4px;">🌊 mardetareas</div>
      <div style="opacity:.7;font-size:13px;">Informe ejecutivo de proyectos</div>
      <div style="opacity:.5;font-size:11px;margin-top:4px;">${projects.join("  ·  ")}</div>
    </div>
    <div style="text-align:right;">
      <div style="font-size:28px;font-weight:800;">${projects.length}</div>
      <div style="opacity:.7;font-size:12px;">proyecto${projects.length!==1?"s":""}</div>
      <div style="opacity:.5;font-size:11px;margin-top:4px;">${today_}</div>
    </div>
  </div>
  ${sections}
...
function Checklist({items,color,th,onChange}){
  ...
  return(<div>
    ...
    {items.map(item=>(<div key={item.id} style={{marginBottom:6}}>
...
function KPIChart({table,th,dark}){
  ...
  const W=700,H=260,PAD={top:24,right:24,bottom:48,left:60};
  const innerW=W-PAD.left-PAD.right;
  const innerH=H-PAD.top-PAD.bottom;
      ...
      <svg width={W} height={H} style={{display:"block",margin:"0 auto",maxWidth:"100%",fontFamily:"inherit"}}>
        ...
        {Object.entries(table.objectives||{}).map(([ci,objV])=>{
          ...
          return(<g key={"obj"+ci}>
            ...
            <text x={PAD.left+innerW-2} y={y-4} textAnchor="end" fontSize={8} fill="#FF4444">Obj {col}: {parseFloat(objV).toLocaleString("es-ES")}</text>
          </g>);
        })}
        {/* Bars or Lines */}
        {isLine?(
          cols.map((col,ci)=>{
            ...
            return(<g key={ci}>
              ...
              {data.map((d,i)=>{
                const x=PAD.left+i*xStep+xStep/2;
                const y=PAD.top+yScale(d[col]||0);
                return<circle key={i} cx={x} cy={y} r={3.5} fill={COLORS[ci%COLORS.length]}/>;
              })}
            </g>);
          })
        ):(
            ...
            return(<g key={i}>
              {cols.map((col,ci)=>{
                ...
                return(<g key={ci}>
                  ...
                  {bh>14&&<text x={bx+barW/2} y={by+10} textAnchor="middle" fontSize={7} fill="#fff" fontWeight={700}>{fmtY(val)}</text>}
                </g>);
              })}
            </g>);
          })
    ...
    <div style={{background:cardBg,borderRadius:14,border:"1px solid "+th.border2,overflow:"hidden",marginBottom:20}}>
      <div style={{padding:"12px 16px",borderBottom:"1px solid "+th.border2,display:"flex",justifyContent:"space-between",alignItems:"center",background:dark?"#151D2B":"#EEF3FB",flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          ...
          <button onClick={()=>setShowGraph(g=>!g)} style={{padding:"5px 10px",borderRadius:7,fontSize:11,fontWeight:600,border:"1px solid "+th.border2,background:"transparent",color:th.text4,cursor:"pointer"}}>{showGraph?"Ver tabla":"Ver grafico"}</button>
          ...
          <button onClick={()=>onUpdate({...table,rows:table.rows.slice(0,-1)})} disabled={table.rows.length===0} style={{padding:"5px 10px",borderRadius:7,fontSize:11,border:"1px solid #FF6B6B44",background:"transparent",color:table.rows.length===0?"#ccc":"#FF6B6B",cursor:table.rows.length===0?"default":"pointer"}}>− Fila</button>
          <button onClick={onDelete} style={{padding:"5px 8px",borderRadius:7,fontSize:11,border:"1px solid #aaa44",background:"transparent",color:"#aaa",cursor:"pointer"}} title="Eliminar tabla completa">🗑</button>
        </div>
      </div>
      {editObj&&(
          ...
          <thead>
            <tr style={{background:dark?"#151D2B":"#EEF3FB"}}>
              {table.columns.map((col,ci)=>(
                <th key={ci} style={{padding:"8px 12px",textAlign:ci===0?"left":"right",fontSize:11,fontWeight:700,color:"#1A3A6E",borderBottom:"1px solid "+th.border2,whiteSpace:"nowrap"}}>{col}</th>
              ))}
                  ...
                  <td key={ci} style={{padding:"2px 4px",borderBottom:"1px solid "+th.border2,textAlign:ci===0?"left":"right"}}>
                    {editCell&&editCell.ri===ri&&editCell.ci===ci?(
                      ...
                      <div onClick={()=>setEditCell({ri,ci})} style={{padding:"6px 8px",borderRadius:6,cursor:"text",fontSize:12,fontWeight:ci===0?700:400,color:ci===0?"#1A3A6E":th.text,minWidth:50}}>
                        {cell||<span style={{color:th.border}}>--</span>}
                      </div>
                    )}
                  </td>
                ))}
...
function KPITab({th,dark,labelBank}){
  const loadKPIs=(proj)=>{try{const v=localStorage.getItem("mdt_kpis_"+proj);return v?JSON.parse(v):null;}catch{return null;}};
  ...
  const initTables=(proj)=>{
    ...
    if(!saved&&proj==="Estandarización FIRMES"){
  ...
  return(
    <div style={{padding:"20px 0"}}>
      ...
      {addingTable?(
        ...
        <button onClick={()=>setAddingTable(true)} style={{width:"100%",padding:"14px",borderRadius:12,border:"1.5px dashed "+th.border,background:"transparent",color:th.text5,fontSize:12,fontWeight:600,cursor:"pointer"}}>
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v50 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
          ...
          {tab==="history"&&(()=>{
            ...
            {histMode!=="project"&&<div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",marginBottom:16,padding:"10px 14px",background:th.surface2,borderRadius:11,border:`1px solid ${th.border2}`}}>
              {histMode==="week"?(<>
                ...
                <button onClick={()=>setHFrom(addDays(hFrom||getWeekStart(today()),-7))||setHTo(addDays(hTo||addDays(getWeekStart(today()),4),-7))} style={{padding:"4px 9px",borderRadius:7,border:`1px solid ${th.border}`,background:th.surface,color:th.text3,cursor:"pointer",fontSize:12}}>‹</button>
                <span style={{fontSize:12,color:th.text2,fontWeight:600}}>{hFrom?`${fmtShort(hFrom)} – ${fmtShort(addDays(hFrom,4))}`:"Selecciona semana"}</span>
                <button onClick={()=>setHFrom(addDays(hFrom||getWeekStart(today()),7))||setHTo(addDays(hTo||addDays(getWeekStart(today()),4),7))} style={{padding:"4px 9px",borderRadius:7,border:`1px solid ${th.border}`,background:th.surface,color:th.text3,cursor:"pointer",fontSize:12}}>›</button>
                <button onClick={()=>{const w=getWeekStart(today());setHFrom(w);setHTo(addDays(w,4));}} style={{padding:"4px 9px",borderRadius:7,border:`1px solid ${th.border}`,background:th.surface,color:th.text3,cursor:"pointer",fontSize:11}}>Esta semana</button>
              </>):(<>
            ...
            {histMode==="project"&&(()=>{
              ...
              return(<div style={{marginBottom:16}}>
                <div style={{padding:"12px 14px",background:th.surface2,borderRadius:11,border:`1px solid ${th.border2}`,marginBottom:12}}>
                  ...
                  {selProjects.length>0&&<button onClick={()=>exportProjectReport(cats,selProjects,dark,Object.fromEntries(selProjects.map(p=>[p,JSON.parse(localStorage.getItem('mdt_kpis_'+p)||'[]')])))} style={{marginTop:12,padding:"8px 16px",borderRadius:9,background:dark?"#0D1E35":"#E8EEF8",border:"1px solid #1A3A6E44",color:"#1A3A6E",fontSize:12,fontWeight:700,cursor:"pointer"}}>⬇ Generar informe ejecutivo (HTML+PDF)</button>}
                </div>
