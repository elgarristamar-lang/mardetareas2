...
const BUILD_TIME="2026-05-14T10:00:00Z";
...
export default function App(){
  ...
  const restoreFromInforme = () => {
    if(!window.confirm("¿Restaurar los datos del proyecto 'Estandarización FIRMES' desde el informe PDF del 25/03/2026?\n\nSe creará (o actualizará) la categoría con 22 tareas:\n• 12 completadas\n• 4 en curso\n• 2 bloqueadas\n• 4 backlog\n\nNo se eliminarán datos existentes.")) return;
    const mkT = (t) => ({
      id: genId(), text: t.text, done: !!t.completedAt, taskKind: "tarea",
      priority: t.priority || "media", status: t.status || "backlog",
      blockReason: t.blockReason || "", createdAt: t.completedAt || today(),
      deadline: t.deadline || null, completedAt: t.completedAt || null,
      recurrence: null, jiraUrl: "", description: "",
      checklist: (t.checklist || []).map(c => ({...c, id: c.id || genId()})),
      contacts: [], labels: t.labels || [], owner: "",
    });
    const allTasks = [
      ...FIRMES_RESTORE_DATA.completed.map(mkT),
      ...FIRMES_RESTORE_DATA.active.map(mkT),
      ...FIRMES_RESTORE_DATA.blocked.map(mkT),
      ...FIRMES_RESTORE_DATA.backlog.map(mkT),
    ];
    setLabelBank(prev => {
      const exists = prev.find(l => l.text === "Estandarización FIRMES");
      return exists ? prev : [...prev, {id: genId(), text: "Estandarización FIRMES", color: "#2E5FA3"}];
    });
    setCatsP(prev => {
      const existing = prev.find(c => c.name === "Estandarización FIRMES");
      if(existing) {
      ...
      return [...prev, {id: genId(), name: "Estandarización FIRMES", icon: "🎯", colorIdx: 0, type: "tasks", tasks: allTasks}];
    });
    alert("✅ Datos restaurados.\n\n• 12 completadas\n• 4 en curso (con checklist)\n• 2 bloqueadas (con motivo)\n• 4 backlog\n\nRevisa 'Estandarización FIRMES' en el sidebar.");
  };
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v1.13 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
          <button onClick={restoreFromInforme} title="Restaurar datos Estandarización FIRMES desde informe PDF 2026-03-25"
