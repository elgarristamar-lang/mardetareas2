...
const BUILD_TIME="2026-03-09T18:06:47Z";
...
function TaskRow({task,color,th,onToggle,onDelete,onUpdate,showInPersonal=false,labelBank=[],onLabelBankChange=()=>{}}){
  ...
  return(<div style={{background:task.done?th.rowDone:th.rowBg,borderRadius:12,marginBottom:8,border:`1px solid ${task.done?th.border3:(dl?.urgent?dl.color+"44":th.border)}`,opacity:task.done?0.55:1,transition:"all 0.2s",boxShadow:dark?"none":"0 1px 4px #0001"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,padding:"11px 13px",cursor:task.done?"default":"pointer"}} onClick={()=>!task.done&&!editT&&setExp(e=>!e)}>
      ...
      {!editT&&!task.done&&(()=>{const tk=TASK_KIND[task.taskKind||"tarea"];return(<span style={{fontSize:10,flexShrink:0,color:th.text5,fontStyle:"italic",whiteSpace:"nowrap"}}>{tk.label}</span>);})()}
...
function CalendarView({th,dark,calData,onCalDataUpdate}){
  ...
  const dragRef=useRef(null); const [dragActive,setDragActive]=useState(null); // dragActive just for opacity re-render
...
// ── Tasks Calendar View ───────────────────────────────────────
function TasksCalendarView({cats,th,dark,onNavigate,onUpdateTask,personalOnly=false}){
  const todayStr=today();
  const [weekStart,setWeekStart]=useState(getWeekStart(todayStr));
  ...
  const allPendingAll=sourceCats.flatMap(c=>c.tasks.filter(t=>!t.done));
  ...
  const seguimientoTasks=allPendingAll.filter(t=>(t.taskKind||"tarea")==="seguimiento");
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
      {weekDates.map(date=>{
        ...
        const dayName=WEEK_DAYS_ES[d.getDay()-1]||"";
        const dayTasks=(tasksByDate[date]||[]).slice().sort((a,b)=>(a.calendarOrder??99999)-(b.calendarOrder??99999));
        const handleDrop=(e)=>{
          e.preventDefault();
          if(!dragRef.current||dragRef.current.date!==date)return;
          const tasks=[...dayTasks];
          const fromIdx=tasks.findIndex(t=>t.id===dragRef.current.id);
          const toIdx=parseInt(e.currentTarget.dataset.dropIdx||tasks.length);
          if(fromIdx<0||fromIdx===toIdx)return;
          const [moved]=tasks.splice(fromIdx,1);
          tasks.splice(toIdx,0,moved);
          tasks.forEach((t,i)=>{if(!t.meetingId&&onUpdateTask)onUpdateTask(t.catId,t.id,{calendarOrder:i});});
          dragRef.current=null;setDragActive(null);
        };
        return(<div key={date} style={{borderRadius:11,background:isToday?dark?"#0D1E35":"#EBF4FF":th.surface2,border:`1.5px solid ${isToday?"#74B9FF55":th.border2}`,overflow:"hidden"}}>
          ...
          <div style={{padding:"6px",minHeight:80}}>
            {dayTasks.length===0?(<div style={{textAlign:"center",paddingTop:20,color:th.text6,fontSize:11}}>—</div>):(
              dayTasks.map((t,idx)=>(<div key={t.id}
                draggable={!t.meetingId}
                onDragStart={e=>{e.dataTransfer.effectAllowed="move";dragRef.current={id:t.id,catId:t.catId,date};setDragActive(t.id);}}
                onDragEnd={()=>{dragRef.current=null;setDragActive(null);}}
                onDragOver={e=>{e.preventDefault();e.currentTarget.style.borderColor=t.catColor.accent;}}
                onDragLeave={e=>{e.currentTarget.style.borderColor=t.catColor.accent+"33";}}
                onDrop={e=>{e.preventDefault();e.stopPropagation();if(!dragRef.current||dragRef.current.date!==date)return;const tasks=[...dayTasks];const fromIdx=tasks.findIndex(x=>x.id===dragRef.current.id);const toIdx=idx;if(fromIdx<0||fromIdx===toIdx)return;const[moved]=tasks.splice(fromIdx,1);tasks.splice(toIdx,0,moved);tasks.forEach((x,i)=>{if(!x.meetingId&&onUpdateTask)onUpdateTask(x.catId,x.id,{calendarOrder:i});});dragRef.current=null;setDragActive(null);}}
                onClick={()=>onNavigate(t.catId)}
                style={{marginBottom:5,padding:"5px 8px",borderRadius:7,background:t.catColor.light,border:`1px solid ${t.catColor.accent}33`,cursor:t.meetingId?"pointer":"grab",transition:"opacity 0.15s",opacity:dragActive===t.id?0.4:1}}
                onMouseEnter={e=>{if(dragActive!==t.id)e.currentTarget.style.opacity="0.8";}} onMouseLeave={e=>{if(dragActive!==t.id)e.currentTarget.style.opacity="1";}}>
                <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
    ...
    {seguimientoTasks.length>0&&(<div style={{marginTop:14,padding:"10px 14px",background:dark?"#1a1a1a":"#F7F7F7",border:`1px solid ${th.border2}`,borderRadius:12}}>
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
        <span style={{fontSize:12,color:th.text5}}>🔍</span>
        <span style={{fontSize:12,color:th.text4,fontWeight:700}}>{seguimientoTasks.length} seguimiento{seguimientoTasks.length!==1?"s":""} pendiente{seguimientoTasks.length!==1?"s":""}</span>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:4}}>
        {[...seguimientoTasks].sort((a,b)=>{if(!a.deadline&&!b.deadline)return 0;if(!a.deadline)return 1;if(!b.deadline)return -1;return a.deadline.localeCompare(b.deadline);}).map(t=>(<div key={t.id} onClick={()=>onNavigate(t.catId)} style={{display:"flex",alignItems:"baseline",gap:8,cursor:"pointer",padding:"2px 4px",borderRadius:6,transition:"background 0.15s"}}
          onMouseEnter={e=>e.currentTarget.style.background=dark?"#ffffff0a":"#0000000a"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
          <span style={{color:th.text5,fontSize:11,flexShrink:0}}>·</span>
          <span style={{fontSize:12,color:th.text2,flex:1}}>{t.text}</span>
          {t.deadline&&<span style={{fontSize:11,color:th.text5,flexShrink:0,whiteSpace:"nowrap"}}>{t.deadline}</span>}
        </div>))}
      </div>
    </div>)}
  </div>);
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v106 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"18px 20px"}}>
          ...
          {/* CALENDAR TAREAS */}
          {tab==="cal-tareas"&&<TasksCalendarView cats={cats} th={th} dark={dark} onNavigate={navigateTo} onUpdateTask={updTask}/>}

          {/* CALENDAR PERSONAL */}
          {tab==="cal-personal"&&<TasksCalendarView cats={cats} th={th} dark={dark} onNavigate={navigateTo} onUpdateTask={updTask} personalOnly={true}/>}
