...
const BUILD_TIME="2026-03-08T02:07:24Z";
...
function mkTask(text){return{id:genId(),text,done:false,priority:"media",status:"backlog",blockReason:"",createdAt:today(),deadline:null,completedAt:null,jiraUrl:"",description:"",recurrence:null,checklist:[],contacts:[],labels:[],owner:""};}
...
function exportProjectReport(cats,projects,dark,kpiData={},projFrom='',projTo='',kpiHighlights={}){
  const today_=today();
  ...
  const buildKPISection=(proj,kpiData,kpiHighlights={})=>{
    const tables=(kpiData[proj])||[];
    if(!tables||tables.length===0)return "";
    const hl=(kpiHighlights||{})[proj]||"";
    const hlBlock=hl?`<div style="background:#F5F8FF;border-left:4px solid #2E5FA3;border-radius:0 8px 8px 0;padding:12px 16px;margin-bottom:20px;">
      <div style="font-size:10px;font-weight:800;color:#2E5FA3;letter-spacing:1px;text-transform:uppercase;margin-bottom:6px;">✏️ Highlights</div>
      <div style="font-size:12px;color:#1A1A2E;line-height:1.7;white-space:pre-wrap;">${hl}</div>
    </div>`:"";
  ...
  const sections=projects.map(proj=>{
    ...
    <div class="kpi-block" style="page-break-inside:avoid;break-inside:avoid;">${buildKPISection(proj,kpiData,kpiHighlights)}</div>
    </div>`;
...
function KPITab({th,dark,labelBank}){
  ...
  const selectProject=(proj)=>{
    ...
    setHighlights(loadHighlights(proj));
  };
  ...
  return(
    <div style={{padding:"20px 0"}}>
      ...
      {/* Highlights / resumen ejecutivo */}
      <div style={{marginBottom:20}}>
        <div style={{fontSize:11,fontWeight:700,color:th.text5,letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>✏️ Highlights / Resumen ejecutivo</div>
        <textarea
          value={highlights}
          onChange={e=>{setHighlights(e.target.value);saveHighlights(selProj,e.target.value);}}
          placeholder={"Escribe aquí los puntos clave, logros o contexto que quieras incluir al inicio del informe de este proyecto..."}
          rows={4}
          style={{width:"100%",background:th.inputBg,border:`1px solid ${th.border}`,borderRadius:10,padding:"10px 14px",
            color:th.text,fontSize:12,lineHeight:1.6,resize:"vertical",outline:"none",
