...
const BUILD_TIME="2026-03-07T19:22:39Z";
  ...
  const buildKPISection=(proj,kpiData)=>{
    ...
    const tablesHtml=tables.map(table=>{
      ...
      Object.entries(table.objectives||{}).forEach(([ci,val])=>{
        ...
        svgParts.push('<rect x="'+(PAD.left+innerW-labelW)+'" y="'+(y-18)+'" width="'+labelW+'" height="16" rx="4" fill="#FF4444"/>');
        svgParts.push('<text x="'+(PAD.left+innerW-labelW/2)+'" y="'+(y-7)+'" text-anchor="middle" font-size="9" font-weight="700" fill="#fff">'+objLabel+'</text>');
      });
      if(isLine){
        cols.forEach((col,ci)=>{
      ...
      const tbody=rows.map((r,i)=>'<tr style="background:'+(i%2===0?"#fff":"#FAFCFF")+'">'+r.map((c,ci)=>'<td style="padding:5px 10px;font-size:12px;border-bottom:1px solid #EEF2F8;font-weight:'+(ci===0?700:400)+';color:'+(ci===0?"#1A3A6E":"#1A1A2E")+'">'+((ci>0&&c&&!isNaN(parseFloat(c.replace(/[.]/g,'').replace(',','.'))))?parseFloat(c.replace(/[.]/g,'').replace(',','.') ).toLocaleString('es-ES'):c)+'</td>').join("")+'</tr>').join("");
    ...
    <div style={{overflowX:"auto",width:"100%"}}>
      <svg width={W} height={H} style={{display:"block",margin:"0 auto",maxWidth:"100%",fontFamily:"inherit"}}>
        ...
        {Object.entries(table.objectives||{}).map(([ci,objV])=>{
          ...
          return(<g key={"obj"+ci}>
            <line x1={PAD.left} x2={PAD.left+innerW} y1={y} y2={y} stroke="#FF4444" strokeWidth={2} strokeDasharray="8,4"/>
            <text x={PAD.left} y={y-4} textAnchor="start" fontSize={9} fontWeight={700} fill="#FF4444" style={{background:"#FF4444"}}>Obj {col}: {parseFloat(objV).toLocaleString("es-ES")}</text>
          </g>);
        })}
        {/* Bars or Lines */}
        {isLine?(
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v53 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
