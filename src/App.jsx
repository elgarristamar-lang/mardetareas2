...
const BUILD_TIME="2026-05-14T11:00:00Z";
...
export default function App(){
  ...
  const restoreFromInforme = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        // Parse the HTML report generated by exportProjectReport
        const parser = new DOMParser();
        const doc = parser.parseFromString(ev.target.result, "text/html");

        const mkT = (text, done, completedAt, status, priority, deadline, blockReason, labelColor, labelText) => ({
          id: genId(), text: text.trim(), done, taskKind: "tarea",
          priority: priority || "media", status: status || "backlog",
          blockReason: blockReason || "", createdAt: completedAt || today(),
          deadline: deadline || null, completedAt: completedAt || null,
          recurrence: null, jiraUrl: "", description: "",
          checklist: [], contacts: [],
          labels: labelText ? [{id: genId(), text: labelText, color: labelColor || "#2E5FA3"}] : [],
          owner: "",
        });

        // Extract project name from header
        const projectHeader = doc.querySelector(".project-section h2, [style*='font-size:22px']");
        const projectName = projectHeader ? projectHeader.textContent.trim() : file.name.replace(/informe_proyectos_|\.html/g,"").trim() || "Proyecto importado";

        // Extract label color from header gradient (fallback blue)
        const headerDiv = doc.querySelector("[style*='linear-gradient']");
        const labelColor = "#2E5FA3";

        const allTasks = [];

        // ── Parse completed tasks ──
        // Look for rows in completadas section — identified by ✓ badge
        const allRows = doc.querySelectorAll("tbody tr");
        allRows.forEach(tr => {
          const cells = tr.querySelectorAll("td");
          if(cells.length < 4) return;
          const taskText = cells[0]?.querySelector("div")?.textContent?.trim() || cells[0]?.textContent?.trim();
          if(!taskText) return;

          const statusCell = cells[3]?.textContent?.trim() || "";
          const prioText = cells[2]?.textContent?.trim()?.toLowerCase() || "media";
          const priority = prioText.includes("alta") ? "alta" : prioText.includes("baja") ? "baja" : "media";

          // Detect completed: has a date badge with ✓
          const badge = cells[3]?.querySelector("span");
          const badgeText = badge?.textContent?.trim() || "";

          if(badgeText.startsWith("✓")) {
            // Completed task — extract date
            const dateMatch = badgeText.match(/(\d{4}-\d{2}-\d{2})/);
            const completedAt = dateMatch ? dateMatch[1] : today();
            allTasks.push(mkT(taskText, true, completedAt, "backlog", priority, null, "", labelColor, projectName));
          } else if(statusCell.includes("Bloqueada") || statusCell.includes("Bloqueado")) {
            // Blocked task
            const motivo = cells[3]?.querySelector("div")?.textContent?.replace("Motivo:","")?.trim() || "";
            // Check for deadline in text
            const dlMatch = tr.innerHTML.match(/(\d{4}-\d{2}-\d{2})/);
            const deadline = dlMatch ? dlMatch[1] : null;
            allTasks.push(mkT(taskText, false, null, "bloqueado", priority, deadline, motivo, labelColor, projectName));
          } else if(statusCell.includes("En curso") || statusCell.includes("Escalado")) {
            const status = statusCell.includes("Escalado") ? "escalado" : "en-curso";
            const dlMatch = tr.innerHTML.match(/(\d{4}-\d{2}-\d{2})/);
            const deadline = dlMatch ? dlMatch[1] : null;
            allTasks.push(mkT(taskText, false, null, status, priority, deadline, "", labelColor, projectName));
          } else {
            // Backlog
            const dlMatch = tr.innerHTML.match(/(\d{4}-\d{2}-\d{2})/);
            const deadline = dlMatch ? dlMatch[1] : null;
            allTasks.push(mkT(taskText, false, null, "backlog", priority, deadline, "", labelColor, projectName));
          }
        });

        if(allTasks.length === 0) {
          alert("No se encontraron tareas en el informe.\n\nAsegúrate de subir un informe HTML generado por mardetareas.");
          return;
        }
        ...
        if(!window.confirm(`¿Importar el informe "${projectName}"?\n\n• ${done} completadas\n• ${active} en curso\n• ${blocked} bloqueadas\n• ${backlog} backlog\n\nTotal: ${allTasks.length} tareas. No se eliminarán datos existentes.`)) return;

        // Update label bank
        setLabelBank(prev => {
          const exists = prev.find(l => l.text === projectName);
          return exists ? prev : [...prev, {id: genId(), text: projectName, color: labelColor}];
        });

        // Add or merge category
        setCatsP(prev => {
        ...
        alert(`✅ Importado correctamente.\n\n"${projectName}"\n• ${done} completadas\n• ${active} en curso\n• ${blocked} bloqueadas\n• ${backlog} backlog\n\nRevisa la categoría en el sidebar.`);
      } catch(err) {
        alert("Error al procesar el informe: " + err.message);
      }
    };
    reader.readAsText(file, "utf-8");
  };
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v1.14 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
          <label title="Importar tareas desde informe HTML generado por mardetareas"
            style={{padding:"5px 9px",borderRadius:99,border:"1px solid #2E5FA366",background:dark?"#0D1E35":"#EEF3FB",color:"#2E5FA3",fontSize:11,cursor:"pointer",fontWeight:700,whiteSpace:"nowrap"}}>
            📥 Importar informe
            <input type="file" accept=".html" style={{display:"none"}} onChange={restoreFromInforme}/>
          </label>
