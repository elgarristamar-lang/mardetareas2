...
const BUILD_TIME="2026-03-09T18:04:58Z";
...
function CalendarView({th,dark,calData,onCalDataUpdate}){
  ...
  const [dragInfo,setDragInfo]=useState(null); // {id,catId,date}
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
      {weekDates.map(date=>{
        ...
        const handleDrop=(e)=>{
          e.preventDefault();
          if(!dragInfo||dragInfo.date!==date)return;
          const tasks=[...dayTasks];
          const fromIdx=tasks.findIndex(t=>t.id===dragInfo.id);
          ...
          <div style={{padding:"6px",minHeight:80}}>
            {dayTasks.length===0?(<div style={{textAlign:"center",paddingTop:20,color:th.text6,fontSize:11}}>—</div>):(
              dayTasks.map((t,idx)=>(<div key={t.id}
                ...
                onDragStart={e=>{e.dataTransfer.effectAllowed="move";setDragInfo({id:t.id,catId:t.catId,date});}}
                ...
                onDrop={e=>{e.preventDefault();e.stopPropagation();if(!dragInfo||dragInfo.date!==date)return;const tasks=[...dayTasks];const fromIdx=tasks.findIndex(x=>x.id===dragInfo.id);const toIdx=idx;if(fromIdx<0||fromIdx===toIdx)return;const[moved]=tasks.splice(fromIdx,1);tasks.splice(toIdx,0,moved);tasks.forEach((x,i)=>{if(!x.meetingId&&onUpdateTask)onUpdateTask(x.catId,x.id,{calendarOrder:i});});setDragInfo(null);}}
                onClick={()=>onNavigate(t.catId)}
                style={{marginBottom:5,padding:"5px 8px",borderRadius:7,background:t.catColor.light,border:`1px solid ${t.catColor.accent}33`,cursor:t.meetingId?"pointer":"grab",transition:"opacity 0.15s",opacity:dragInfo?.id===t.id?0.4:1}}
                onMouseEnter={e=>{if(dragInfo?.id!==t.id)e.currentTarget.style.opacity="0.8";}} onMouseLeave={e=>{if(dragInfo?.id!==t.id)e.currentTarget.style.opacity="1";}}>
                <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v105 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
