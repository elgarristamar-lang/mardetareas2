...
const BUILD_TIME="2026-03-06T06:37:56Z";
...
async function exportHistorialXLSX(doneTasks){
  ...
  // Sheet 1: Raw data
  const rows=[["Tarea","Categoría","Prioridad","Fecha Inicio","Fecha Fin","Deadline","Días activa"],
    ...doneTasks.map(t=>{
  ...
  const ws1=utils.aoa_to_sheet(rows);
  ws1["!cols"]=[{wch:40},{wch:16},{wch:10},{wch:14},{wch:14},{wch:14},{wch:12}];
  ...
  doneTasks.forEach(t=>{
    ...
    if(t.priority)byCat[t.catName][t.priority]=(byCat[t.catName][t.priority]||0)+1;
    const ini=t.createdAt?new Date(t.createdAt+"T00:00:00"):null;
    const fin=t.completedAt?new Date(t.completedAt+"T00:00:00"):null;
  ...
  const dash=[
    ["📊 DASHBOARD - HISTORIAL DE TAREAS","","","","",""],
    ["Generado:",today(),"","","",""],
    ["","","","","",""],
    ["── POR CATEGORÍA ──","","","","",""],
    ["Categoría","Completadas","Alta","Media","Baja","Días prom."],
    ...Object.entries(byCat).sort((a,b)=>b[1].total-a[1].total).map(([cat,v])=>[
      cat,v.total,v.alta||0,v.media||0,v.baja||0,
      v.conDias>0?Math.round(v.diasTotal/v.conDias)+"d":"—"
    ]),
    ["","","","","",""],
    ["── POR MES ──","","","","",""],
    ["Mes","Tareas completadas","","","",""],
    ...Object.entries(byMonth).sort().map(([m,n])=>[m,n,"","","",""]),
    ["","","","","",""],
    ["── RESUMEN GLOBAL ──","","","","",""],
    ["Total completadas",doneTasks.length,"","","",""],
    ["Alta prioridad",doneTasks.filter(t=>t.priority==="alta").length,"","","",""],
    ["Media prioridad",doneTasks.filter(t=>t.priority==="media").length,"","","",""],
    ["Baja prioridad",doneTasks.filter(t=>t.priority==="baja").length,"","","",""],
  ];
  const ws2=utils.aoa_to_sheet(dash);
  ws2["!cols"]=[{wch:22},{wch:18},{wch:10},{wch:10},{wch:10},{wch:12}];
  // Bold header rows
  ["A1","A4","A8","A11"].forEach(cell=>{if(ws2[cell])ws2[cell].s={font:{bold:true,sz:12}};});
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v29 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
