...
const BUILD_TIME="2026-03-09T17:56:28Z";
...
function TasksCalendarView({cats,th,dark,onNavigate,personalOnly=false}){
  ...
  return(<div>
    ...
    {seguimientoTasks.length>0&&(<div style={{marginTop:14,background:dark?"#0A1F1A":"#F0FAF7",border:"1px solid #00B89433",borderRadius:12,overflow:"hidden"}}>
      <div style={{padding:"8px 14px",display:"flex",alignItems:"center",gap:8,borderBottom:`1px solid ${dark?"#00B89433":"#00B89422"}`}}>
      ...
      <div style={{padding:"8px 10px",display:"flex",flexWrap:"wrap",gap:6}}>
        {seguimientoTasks.map(t=>{
          const cat=sourceCats.find(c=>c.id===t.catId);
          const color=cat?gc(COLORS[cat.colorIdx],dark):{light:"#e8faf5",accent:"#00B894",tc:"#00916e"};
          return(<div key={t.id} onClick={()=>onNavigate(t.catId)} style={{padding:"6px 10px",borderRadius:8,background:color.light,border:`1px solid ${color.accent}33`,cursor:"pointer",minWidth:160,maxWidth:280,transition:"opacity 0.15s"}}
            onMouseEnter={e=>e.currentTarget.style.opacity="0.8"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
            <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
              <span style={{fontSize:10}}>{t.catIcon}</span>
              <span style={{fontSize:10,color:color.tc,fontWeight:600}}>{t.catName}</span>
              {t.deadline&&<span style={{fontSize:9,color:th.text5,marginLeft:"auto"}}>📅 {t.deadline}</span>}
            </div>
            <div style={{fontSize:11.5,color:th.text2,lineHeight:1.3}}>{t.text}</div>
          </div>);
        })}
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v102 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
