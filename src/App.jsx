...
const BUILD_TIME="2026-03-09T18:12:06Z";
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
      {weekDates.map(date=>{
        ...
        const dayName=WEEK_DAYS_ES[d.getDay()-1]||"";
        const dayTasks=(tasksByDate[date]||[]).slice().sort((a,b)=>((a.calendarOrder==null?99999:a.calendarOrder)-(b.calendarOrder==null?99999:b.calendarOrder)));
        return(
          <div key={date} style={{borderRadius:11,background:isToday?dark?"#0D1E35":"#EBF4FF":th.surface2,border:"1.5px solid "+(isToday?"#74B9FF55":th.border2),overflow:"hidden"}}>
            ...
            <div style={{padding:"6px",minHeight:80}}>
              {dayTasks.length===0
                ?<div style={{textAlign:"center",paddingTop:20,color:th.text6,fontSize:11}}>—</div>
                :dayTasks.map((t,idx)=>{
                  const doReorder=(toIdx)=>{
                    const arr=[...dayTasks];
                    const from=arr.findIndex(x=>x.id===dragRef.current.id);
                    if(from<0||from===toIdx)return;
                    const[mv]=arr.splice(from,1);
                    arr.splice(toIdx,0,mv);
                    arr.forEach((x,i)=>{ if(!x.meetingId&&onUpdateTask){onUpdateTask(x.catId,x.id,{calendarOrder:i});} });
                  ...
                  return(
                    <div key={t.id}
                      ...
                      onDragOver={e=>{ e.preventDefault(); e.currentTarget.style.borderTopColor=t.catColor.accent; }}
                      onDragLeave={e=>{ e.currentTarget.style.borderTopColor="transparent"; }}
                      onDrop={e=>{ e.preventDefault(); e.stopPropagation(); if(!dragRef.current||dragRef.current.date!==date)return; doReorder(idx); }}
                      onClick={()=>onNavigate(t.catId)}
                      style={{marginBottom:5,padding:"5px 8px",borderRadius:7,background:t.catColor.light,border:"1px solid "+t.catColor.accent+"33",cursor:t.meetingId?"pointer":"grab",opacity:dragActive===t.id?0.4:1}}
                      onMouseEnter={e=>{ if(dragActive!==t.id)e.currentTarget.style.opacity="0.8"; }}
                      onMouseLeave={e=>{ if(dragActive!==t.id)e.currentTarget.style.opacity="1"; }}>
                      <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v107 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
