...
const BUILD_TIME="2026-03-07T18:44:37Z";
...
function KPITab({th,dark,labelBank}){
  ...
  const [selProj,setSelProj]=useState(labelBank.length>0?labelBank[0].text:null);
  const [tables,setTables]=useState(()=>{
    const first=labelBank.length>0?labelBank[0].text:null;
    if(!first)return[];
    let saved=loadKPIs(first);
    if(!saved&&first==="Estandarización FIRMES"){saved=FIRMES_DEFAULT_TABLES;saveKPIs(first,saved);}
    return saved||[];
  });
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v41 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
