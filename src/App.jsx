...
const BUILD_TIME="2026-03-07T17:34:54Z";
...
<div class="page">
  ...
  <div class="card">
    ...
    <div style="display:flex;height:32px;border-radius:99px;overflow:hidden;margin-bottom:14px;box-shadow:inset 0 1px 4px #0001;">
      ${alta>0?`<div style="width:${Math.round((alta/doneTasks.length)*100)}%;background:#FF6B6B;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#fff;">${Math.round((alta/doneTasks.length)*100)}%</div>`:""}
      ${media>0?`<div style="width:${Math.round((media/doneTasks.length)*100)}%;background:#FFD93D;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#7A6000;">${Math.round((media/doneTasks.length)*100)}%</div>`:""}
      ${baja>0?`<div style="width:${Math.round((baja/doneTasks.length)*100)}%;background:#6BCB77;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#fff;">${Math.round((baja/doneTasks.length)*100)}%</div>`:""}
    </div>
    <div style="display:flex;gap:20px;flex-wrap:wrap;">
      ${alta>0?`<span style="display:flex;align-items:center;gap:7px;font-size:13px;font-weight:600;"><span style="width:14px;height:14px;border-radius:3px;background:#FF6B6B;display:inline-block;"></span>Alta &nbsp;<strong style="color:#CC2222">${alta}</strong> (${Math.round((alta/doneTasks.length)*100)}%)</span>`:""}
      ${media>0?`<span style="display:flex;align-items:center;gap:7px;font-size:13px;font-weight:600;"><span style="width:14px;height:14px;border-radius:3px;background:#FFD93D;display:inline-block;"></span>Media &nbsp;<strong style="color:#AA8800">${media}</strong> (${Math.round((media/doneTasks.length)*100)}%)</span>`:""}
      ${baja>0?`<span style="display:flex;align-items:center;gap:7px;font-size:13px;font-weight:600;"><span style="width:14px;height:14px;border-radius:3px;background:#6BCB77;display:inline-block;"></span>Baja &nbsp;<strong style="color:#2A7A34">${baja}</strong> (${Math.round((baja/doneTasks.length)*100)}%)</span>`:""}
    </div>`:"<p style='color:#aaa'>Sin datos</p>"}
  </div>
...
function Labels({labels=[],th,onChange}){
  const [adding,setAdding]=useState(false);const [nw,setNw]=useState("");const [color,setColor]=useState(LABEL_PALETTE[0]);
  ...
  return(<div>
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:6}}>
      {labels.map(lb=>(<span key={lb.id} style={{display:"inline-flex",alignItems:"center",gap:4,padding:"2px 8px",borderRadius:99,fontSize:11,fontWeight:600,background:lb.color+"22",color:lb.color,border:`1px solid ${lb.color}44`}}>{lb.text}<span onClick={()=>onChange(labels.filter(l=>l.id!==lb.id))} style={{cursor:"pointer",fontSize:10,opacity:0.6}}>✕</span></span>))}
    ...
    {adding&&(<div style={{display:"flex",flexDirection:"column",gap:7,background:th.subBg,borderRadius:9,padding:"9px 10px",border:`1px solid ${th.border2}`}}>
...
export default function App(){
  ...
  const allTasks=cats.flatMap(c=>c.tasks.map(t=>{
    ...
    if(!text&&(c.type==="meeting"||c.type==="meeting121eq")){
      ...
      const pts=chk.map(i=>i.text).filter(Boolean);
      if(c.type==="meeting121eq"&&t.collaborator){
        text=`1:1 ${t.collaborator}${pts.length>0?" · "+pts.slice(0,2).join(", "):""}`;
      } else {
        text=`Reunión ${t.meetingId||""}${pts.length>0?" · "+pts.slice(0,2).join(", "):""}`;
      }
    }
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v33 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
