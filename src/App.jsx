...
const BUILD_TIME="2026-03-08T00:08:15Z";
...
function exportProjectReport(cats,projects,dark,kpiData={},projFrom='',projTo=''){
  ...
  const sections=projects.map(proj=>{
    ...
    // Build Gantt
    const mkGantt=(()=>{
      const allT=[...active,...blocked,...backlog].filter(t=>t.deadline||t.createdAt);
      if(allT.length===0)return "";
      const today_d=new Date();
      const dates=allT.map(t=>new Date(t.deadline||t.createdAt));
      ...
      const W=900,ROW_H=32,LABEL_W=300,PAD_TOP=40,PAD_BOT=28;
      const chartW=W-LABEL_W;
      const H=PAD_TOP+allT.length*ROW_H+PAD_BOT;
      const xOff=d=>((new Date(d)-minD)/(1000*60*60*24))/totalDays*chartW;
      ...
      while(cur<=maxD){
        ...
        months.push(`<line x1="${x}" x2="${x}" y1="${PAD_TOP-10}" y2="${H-PAD_BOT}" stroke="#E0E8F4" stroke-width="1"/>`);
        months.push(`<text x="${x+4}" y="${PAD_TOP-14}" font-size="9" fill="#7A8AA0">${cur.toLocaleDateString("es-ES",{month:"short",year:"2-digit"})}</text>`);
        cur.setMonth(cur.getMonth()+1);
      }

      // Today line
      const todayX=LABEL_W+xOff(today_d);
      const todayLine=`<line x1="${todayX}" x2="${todayX}" y1="${PAD_TOP-10}" y2="${H-PAD_BOT}" stroke="#FF4444" stroke-width="1.5" stroke-dasharray="4,3"/>
        <text x="${todayX+3}" y="${PAD_TOP-14}" font-size="9" font-weight="700" fill="#FF4444">Hoy</text>`;
      ...
      const bars=allT.map((t,i)=>{
        ...
        const startD=new Date(t.createdAt||today_d);
        const endD=new Date(t.deadline||t.createdAt||today_d);
        const x1=LABEL_W+xOff(startD);
        const x2=LABEL_W+xOff(endD);
        const barW=Math.max(x2-x1,6);
        const words=t.text.split(" ");
        let ln1="",ln2="";
        for(const w of words){if((ln1+" "+w).trim().length<=30)ln1=(ln1+" "+w).trim();else if((ln2+" "+w).trim().length<=30)ln2=(ln2+" "+w).trim();}
        if(!ln2&&t.text.length>30){ln1=t.text.slice(0,28)+"…";ln2="";}
        const labelSVG=ln2
          ?`<text text-anchor="end" font-size="10" fill="#1A3A6E" font-weight="${t.done?400:600}"><tspan x="${LABEL_W-6}" y="${y+ROW_H/2-2}">${ln1}</tspan><tspan x="${LABEL_W-6}" dy="13">${ln2}</tspan></text>`
          :`<text x="${LABEL_W-6}" y="${y+ROW_H/2+4}" text-anchor="end" font-size="11" fill="#1A3A6E" font-weight="${t.done?400:600}">${ln1}</text>`;
        const isLate=!t.done&&t.deadline&&new Date(t.deadline)<today_d;
        return `<g>
          ${labelSVG}
          <rect x="${x1}" y="${y+4}" width="${barW}" height="${ROW_H-8}" rx="4" fill="${color}" opacity="${t.done?0.4:0.85}"/>
          ${isLate?`<text x="${x1+barW+3}" y="${y+ROW_H/2+4}" font-size="9" fill="#FF4444">⚠</text>`:""}
          ${t.deadline?`<text x="${x1+barW+3}" y="${y+ROW_H/2+4}" font-size="8" fill="#888">${isLate?"":t.deadline}</text>`:""}
        </g>`;
      }).join("");

      // Legend
      const legendItems=[["#2E5FA3","Backlog"],["#FF9F43","En curso"],["#9966FF","Escalado"],["#FF4444","Bloqueado"],["#6BCB77","Completado"]];
      const legend=legendItems.map(([ c,l],i)=>`<g transform="translate(${LABEL_W+i*110},${H-6})"><rect x="0" y="-7" width="10" height="10" fill="${c}" rx="2" opacity=".85"/><text x="14" y="4" font-size="9" fill="#7A8AA0">${l}</text></g>`).join("");
      ...
      return `<div class="report-section" style="margin-bottom:24px;page-break-inside:avoid;break-inside:avoid;">
        ...
        <div style="overflow-x:auto;">
          <svg width="${W}" height="${H}" style="display:block;font-family:inherit;">
            ...
            <rect x="${LABEL_W}" y="${PAD_TOP-10}" width="${chartW}" height="${H-PAD_TOP+10-PAD_BOT}" fill="#F5F8FF"/>
            ${months.join("")}
            ${todayLine}
            ${bars}
            ${legend}
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v72 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
