...
const BUILD_TIME="2026-03-08T01:20:22Z";
  ...
  const sections=projects.map(proj=>{
    ...
    const mkGantt=(()=>{
      ...
      while(cur<=rangeEnd){
        ...
        weeks.push(`<text x="${x+weekW/2}" y="16" text-anchor="middle" font-size="9" font-weight="700" fill="#1A3A6E">S${wNum}</text>`);
        weeks.push(`<text x="${x+weekW/2}" y="28" text-anchor="middle" font-size="8" fill="#9AAABF">${dd}/${mm}</text>`);
        // Vertical separator
        weeks.push(`<line x1="${x}" x2="${x}" y1="${HEADER_H}" y2="${H-PAD_BOT}" stroke="#D8E4F0" stroke-width="1"/>`);
      ...
      while(mCur<=rangeEnd){
        ...
        months.push(`<text x="${x+4}" y="${HEADER_H-12}" font-size="10" font-weight="700" fill="#1A3A6E">${mCur.toLocaleDateString("es-ES",{month:"short",year:"2-digit"}).toUpperCase()}</text>`);
        months.push(`<line x1="${x}" x2="${x}" y1="0" y2="${HEADER_H}" stroke="#C8D8EC" stroke-width="1.5"/>`);
        mCur.setMonth(mCur.getMonth()+1);
      }

      // Today line
      const todayX=LABEL_W+xOff(today_d);
      const todayLine=`<line x1="${todayX}" x2="${todayX}" y1="0" y2="${H-PAD_BOT}" stroke="#FF4444" stroke-width="1.5" stroke-dasharray="4,3"/>
        <text x="${todayX+3}" y="${HEADER_H-2}" font-size="9" font-weight="700" fill="#FF4444">Hoy</text>`;
      ...
      return `<div class="report-section" style="margin-bottom:24px;page-break-inside:avoid;break-inside:avoid;">
        ...
        <div style="overflow-x:auto;">
          <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMinYMin meet" style="display:block;font-family:inherit;border-radius:8px;border:1px solid #E0E8F4;max-width:100%;height:auto;">
            ...
            <rect x="${LABEL_W}" y="0" width="${CHART_W}" height="${HEADER_H}" fill="#EEF3FB"/>
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v93 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
