...
const BUILD_TIME="2026-03-07T18:48:40Z";
...
function exportProjectReport(cats,projects,dark){
  const today_=today();
...
function KPIChart({table,th,dark}){
  const {LineChart,BarChart,Bar,Line,XAxis,YAxis,CartesianGrid,Tooltip,Legend,ReferenceLine,ResponsiveContainer}=Recharts;
    ...
    return(
      <ResponsiveContainer width="100%" height={220}>
        <LineChart data={data} margin={{top:10,right:20,left:0,bottom:0}}>
          ...
          <XAxis dataKey="name" tick={{fontSize:11,fill:th.text4}}/>
          <YAxis tick={{fontSize:11,fill:th.text4}} tickFormatter={fmtNum}/>
          <Tooltip formatter={(v)=>v.toLocaleString("es-ES")} contentStyle={{background:th.surface,border:"1px solid "+th.border,borderRadius:8,fontSize:11}}/>
          <Legend wrapperStyle={{fontSize:11}}/>
          {table.graphCols.map((ci,i)=>{
            const objVal=table.objectives&&table.objectives[ci];
            return([
              <Line key={ci} type="monotone" dataKey={table.columns[ci]} stroke={BAR_COLORS[i%BAR_COLORS.length]} strokeWidth={2} dot={{r:4}}/>,
              objVal?<ReferenceLine key={"ref"+ci} y={objVal} stroke="#FF4444" strokeDasharray="6 3" label={{value:"Obj: "+objVal.toLocaleString("es-ES"),fill:"#FF4444",fontSize:10}}/>:null
            ]);
          })}
        </LineChart>
      </ResponsiveContainer>
  ...
  return(
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={data} margin={{top:10,right:20,left:0,bottom:0}}>
        <CartesianGrid strokeDasharray="3 3" stroke={th.border}/>
        <XAxis dataKey="name" tick={{fontSize:10,fill:th.text4}}/>
        <YAxis tick={{fontSize:10,fill:th.text4}} tickFormatter={fmtNum}/>
        <Tooltip formatter={(v)=>v.toLocaleString("es-ES")} contentStyle={{background:th.surface,border:"1px solid "+th.border,borderRadius:8,fontSize:11}}/>
        <Legend wrapperStyle={{fontSize:11}}/>
        {table.graphCols.map((ci,i)=>{
          const objVal=table.objectives&&table.objectives[ci];
          return([
            <Bar key={ci} dataKey={table.columns[ci]} fill={BAR_COLORS[i%BAR_COLORS.length]} radius={[3,3,0,0]} maxBarSize={26}/>,
            objVal?<ReferenceLine key={"ref"+ci} y={objVal} stroke="#FF4444" strokeDasharray="6 3" label={{value:"Obj "+table.columns[ci]+": "+objVal.toLocaleString("es-ES"),fill:"#FF4444",fontSize:10,position:"insideTopRight"}}/>:null
          ]);
        })}
      </BarChart>
    </ResponsiveContainer>
  ...
  return(
    <div style={{background:cardBg,borderRadius:14,border:"1px solid "+th.border2,overflow:"hidden",marginBottom:20}}>
      <div style={{padding:"12px 16px",borderBottom:"1px solid "+th.border2,display:"flex",justifyContent:"space-between",alignItems:"center",background:dark?"#151D2B":"#EEF3FB",flexWrap:"wrap",gap:8}}>
        <div>
          <div style={{fontSize:13,fontWeight:800,color:"#1A3A6E"}}>{table.name}</div>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          ...
          <button onClick={onDelete} style={{padding:"5px 8px",borderRadius:7,fontSize:11,border:"1px solid #FF6B6B44",background:"transparent",color:"#FF6B6B",cursor:"pointer"}}>Eliminar</button>
        </div>
      </div>
      {editObj&&(
...
function KPITab({th,dark,labelBank}){
  ...
  // Merge labelBank with hardcoded FIRMES project so KPIs always work
  const firmesInBank=labelBank.find(l=>l.text==="Estandarización FIRMES");
  const allProjects=firmesInBank?labelBank:[{text:"Estandarización FIRMES",color:"#2E5FA3",id:"builtin-firmes"},...labelBank];

  // Auto-select first project on mount
  useEffect(()=>{
    const first=allProjects[0]?.text;
    if(first&&!selProj){
      setSelProj(first);
      let saved=loadKPIs(first);
      if(!saved&&first==="Estandarización FIRMES"){saved=JSON.parse(JSON.stringify(FIRMES_DEFAULT_TABLES));saveKPIs(first,saved);}
      setTables(saved||[]);
    }
  },[]);
  ...
  const selectProject=(proj)=>{
    ...
    if(!saved&&proj==="Estandarización FIRMES"){saved=[...FIRMES_DEFAULT_TABLES];saveKPIs(proj,saved);}
    setTables(saved||[]);
  };
  const updateTable=(idx,updated)=>{
    const next=tables.map((t,i)=>i===idx?updated:t);
    setTables(next);if(selProj)saveKPIs(selProj,next);
  };
  ...
  return(
    <div style={{padding:"20px 0"}}>
      <div style={{marginBottom:20}}>
        ...
        <p style={{color:th.text5,fontSize:11,margin:"3px 0 0"}}>Metricas por proyecto</p>
      </div>
      {allProjects.length===0?(
        <div style={{textAlign:"center",padding:"60px 20px",color:th.text5}}>
          <div style={{fontSize:32,marginBottom:12}}>{"📊"}</div>
          <div style={{fontSize:14,fontWeight:600,marginBottom:6}}>No hay proyectos creados</div>
          <div style={{fontSize:12}}>Crea etiquetas de proyecto en tus tareas para verlas aqui.</div>
        </div>
      ):(
        <>
            ...
            <>
              ...
              {tables.length===0&&!addingTable&&(
                <div style={{textAlign:"center",padding:"40px 20px",color:th.text5,background:cardBg,borderRadius:14,border:"1px dashed "+th.border,marginBottom:16}}>
                  <div style={{fontSize:12}}>No hay tablas KPI para este proyecto aun.</div>
                </div>
              )}
              {addingTable?(
                <div style={{background:cardBg,borderRadius:12,border:"1px solid "+th.border2,padding:"16px 18px",marginBottom:16}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    <input value={newName} onChange={e=>setNewName(e.target.value)} placeholder="Nombre (ej: Tiempo medio almacen)"
                    ...
                    <input value={newCols} onChange={e=>setNewCols(e.target.value)} placeholder="Columnas: PERIODO, Metrica1, Metrica2 (separadas por coma)"
                      style={{padding:"8px 12px",borderRadius:8,border:"1px solid "+th.border2,background:th.surface,color:th.text,fontSize:12,outline:"none"}}/>
                    <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                      <span style={{fontSize:11,color:th.text5}}>Grafico:</span>
                      {["bar","line"].map(tp=>(
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v43 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
            ...
            {histMode==="project"&&(()=>{
              ...
              return(<div style={{marginBottom:16}}>
                <div style={{padding:"12px 14px",background:th.surface2,borderRadius:11,border:`1px solid ${th.border2}`,marginBottom:12}}>
                  ...
                  {selProjects.length>0&&<button onClick={()=>exportProjectReport(cats,selProjects,dark)} style={{marginTop:12,padding:"8px 16px",borderRadius:9,background:dark?"#0D1E35":"#E8EEF8",border:"1px solid #1A3A6E44",color:"#1A3A6E",fontSize:12,fontWeight:700,cursor:"pointer"}}>⬇ Generar informe ejecutivo (HTML+PDF)</button>}
                </div>
