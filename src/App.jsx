...
const BUILD_TIME="2026-03-09T17:58:37Z";
...
function TasksCalendarView({cats,th,dark,onNavigate,personalOnly=false}){
  ...
  return(<div>
    ...
    {seguimientoTasks.length>0&&(<div style={{marginTop:14,padding:"10px 14px",background:dark?"#0A1F1A":"#F0FAF7",border:"1px solid #00B89433",borderRadius:12}}>
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
        <span style={{fontSize:12}}>🔍</span>
        <span style={{fontSize:12,color:"#00B894",fontWeight:700}}>{seguimientoTasks.length} seguimiento{seguimientoTasks.length!==1?"s":""} pendiente{seguimientoTasks.length!==1?"s":""}</span>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:4}}>
        {seguimientoTasks.map(t=>(<div key={t.id} onClick={()=>onNavigate(t.catId)} style={{display:"flex",alignItems:"baseline",gap:8,cursor:"pointer",padding:"2px 4px",borderRadius:6,transition:"background 0.15s"}}
          onMouseEnter={e=>e.currentTarget.style.background=dark?"#ffffff11":"#00B89411"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
          <span style={{color:"#00B894",fontSize:11,flexShrink:0}}>·</span>
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v103 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
