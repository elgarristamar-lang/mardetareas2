...
const BUILD_TIME="2026-03-07T19:35:30Z";
...
function exportProjectReport(cats,projects,dark,kpiData={},projFrom='',projTo=''){
  ...
  const buildKPISection=(proj,kpiData)=>{
    ...
    const tablesHtml=tables.map(table=>{
      ...
      const W=860,H=320,PAD={top:24,right:200,bottom:50,left:62};
      ...
      Object.entries(table.objectives||{}).forEach(([ci,val])=>{
        ...
        const objLabel='Obj '+table.columns[parseInt(ci)]+': '+parseFloat(val).toLocaleString("es-ES");
        const labelW=objLabel.length*5.5+10;
        svgParts.push('<line x1="'+PAD.left+'" x2="'+(PAD.left+innerW)+'" y1="'+y+'" y2="'+y+'" stroke="#FF4444" stroke-width="2" stroke-dasharray="8,4"/>');
        svgParts.push('<line x1="'+(PAD.left+innerW)+'" x2="'+(PAD.left+innerW+8)+'" y1="'+y+'" y2="'+y+'" stroke="#FF4444" stroke-width="1.5"/>');
        svgParts.push('<rect x="'+(PAD.left+innerW+10)+'" y="'+(y-9)+'" width="162" height="18" rx="4" fill="#FF444415" stroke="#FF4444" stroke-width="1"/>');
        svgParts.push('<text x="'+(PAD.left+innerW+16)+'" y="'+(y+4)+'" text-anchor="start" font-size="10" font-weight="700" fill="#FF4444">'+objLabel+'</text>');
      });
      if(isLine){
        cols.forEach((col,ci)=>{
      ...
      const nonEmptyCols=table.columns.map((_,ci)=>ci).filter(ci=>ci===0||rows.some(r=>r[ci]&&r[ci].trim()!==""));
      const visibleCols=nonEmptyCols.map(ci=>table.columns[ci]);
      const thead='<tr style="background:#EEF3FB">'+visibleCols.map(c=>'<th style="padding:6px 10px;text-align:'+( visibleCols.indexOf(c)===0?'left':'right')+';font-size:11px;color:#1A3A6E;font-weight:700;border-bottom:1px solid #E0E8F4">'+c+'</th>').join('')+'</tr>';
      const fmtN=(c)=>{const n=parseFloat((c||'').replace(/\./g,'').replace(',','.'));return(!isNaN(n)&&String(c).trim()!=='')?n.toLocaleString('es-ES'):c;};
      const tbody=rows.map((r,i)=>'<tr style="background:'+(i%2===0?'#fff':'#FAFCFF')+'">'+nonEmptyCols.map((ci,j)=>'<td style="padding:5px 10px;font-size:12px;border-bottom:1px solid #EEF2F8;font-weight:'+(j===0?700:400)+';color:'+(j===0?'#1A3A6E':'#1A1A2E')+'">'+( j===0?r[ci]:fmtN(r[ci]))+'</td>').join('')+'</tr>').join('');-ES'):c)+'</td>').join("")+'</tr>').join("");
...
function KPIChart({table,th,dark}){
  ...
  const W=700,H=260,PAD={top:24,right:180,bottom:48,left:60};
  const innerW=W-PAD.left-PAD.right;
  const innerH=H-PAD.top-PAD.bottom;
    ...
    <div style={{minHeight:"100vh",background:th.bg,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",transition:"background 0.2s"}}>
      ...
      <div style={{padding:"12px 16px",borderBottom:`1px solid ${th.border3}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:th.sidebar,boxShadow:dark?"none":"0 1px 6px #0001",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:10,fontWeight:700,background:"#6633CC",padding:"3px 9px",borderRadius:6,flexShrink:0,whiteSpace:"nowrap"}} title="Último deploy">🕐 v58 · {BUILD_TIME.slice(5,10)} {BUILD_TIME.slice(11,16)}</span>
